...
Les configurations (salons, tickets ouverts, reaction-roles, avertissements...) sont stockées dans `data/database.json`. Sur le plan **gratuit** de Render, le disque n'est **pas persistant** : il est réinitialisé à chaque redéploiement/redémarrage du service. Pour un usage sérieux à long terme, il faudra migrer vers une vraie base de données (ex: MongoDB Atlas gratuit, ou un disque persistant Render payant). Dis-le-moi si tu veux que je fasse cette migration.
